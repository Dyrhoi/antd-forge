...
```tsx
import { createTableQueryOptions, useTable } from "antd-forge";
import { queryOptions } from "@tanstack/react-query";
...
Since `useTable` is built on `useForm`, it inherits the `autoSubmit` option. This is particularly useful for filter forms where you want the table to update as the user types—no submit button required. See the [Auto Submit example](#auto-submit) for a live demo.
